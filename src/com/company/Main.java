...
public class Main {
    ...
    public static void golemHit() {
        for (int i = 0; i < heroesAttackType.length; i++) {
            if (heroesAttackType[i] == heroesAttackType[4]) {
                continue;
            }
            if (heroesHealth[i] > 0 && heroesHealth[4] > 0) {
                heroesHealth[i] += bossDamage / 5;
                heroesHealth[4] -= bossDamage / 5;
                System.out.println("Голем взял на себя 1/5 часть урона");
            }
        }
    }
    ...
    public static void lucky() {
        ...
        boolean baff = random.nextBoolean();
        for (int i = 0; i < heroesDamage.length; i++) {
            if (baff == true && heroesHealth[5] > 0) {
                System.out.println("Увернулся от удара");
                heroesHealth[5] += 40;
                break;
            }
        }
